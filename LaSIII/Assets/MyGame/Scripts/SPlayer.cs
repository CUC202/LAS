...
public class SPlayer : MonoBehaviour
{
    ...
    void JumpCheck()
    {
        if (grounded && Input.GetButton("SJump"))
        {
            //确认跳跃状态
            jump = true;
            //确认跳跃动作（Jump）是初次启动
    ...
    void CombineCheck()
    {
        if (Input.GetAxis("SCombine") == 1 && lPlayer.GetComponent<LPlayer>().status == LPlayer.Status.normal)//附身队友
        {
    ...
    void ShootCheck()
    {
        if (Input.GetButton("SShoot") && lPlayer.GetComponent<LPlayer>().status == LPlayer.Status.combined)
        {
            status = Status.shoot;
            Time.timeScale = 0.0f;
    ...
    void CombiningCtrl()
    {
        ...
        if (Vector2.Distance(rBody.position, lPlayer.transform.position) < 0.1f)//到达目标位置（距离足够小）后进入combined状态
        {
            Combine();
        }
    }
    ...
    {
        //跟随目标移动
        rBody.velocity = Vector2.Lerp(rBody.velocity, new Vector2(lPlayer.transform.position.x - transform.position.x,
                                                                  lPlayer.transform.position.y - transform.position.y).normalized * combineSpeed, 1.0f);
    }
    // Shoot状态
    void ShootCtrl()
    {
        ...
        if (Input.GetButtonUp("SShoot"))
        {
            lPlayer.GetComponent<LPlayer>().shootAim = direction;
            lPlayer.GetComponent<LPlayer>().status = LPlayer.Status.bullet;
            lPlayer.GetComponent<LPlayer>().bulletInitial = true;
